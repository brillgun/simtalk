<template>
  <v-container>
    ...
    <v-main class="" style="height: 100%; position: relative">
      <v-card elevation="0">
        <h6 @click="setScrollTo(1)">제1장: 총칙</h6>
        <h6 @click="setScrollTo(2)">제2장: 서비스 이용계약</h6>
        <h6 @click="setScrollTo(3)">제3장: 계약당사자의 의무</h6>
        <h6 @click="setScrollTo(4)">제4장: 서비스 이용</h6>
        <h6 @click="setScrollTo(5)">제5장: 계약해지 및 이용제한</h6>
        <h6 @click="setScrollTo(6)">제6장: 기타</h6>
        <p class="mt-4 mb-0" ref="PAGE1">제1장 총 칙</p>
        <span>
          제1조(목적)<br/>
          이 약관은 애플리케이션 {{ appNm }}(SIMTALK)(이하 "<span class="purple--text">{{ appNm }}</span>" 이라 한다)이 제공하는 모든 서비스(이하 "서비스"라 한다)의 이용조건 및 절차에 관한 사항을 규정함을 목적으로 합니다.<br/>
          제2조(정의)<br/>
          이 약관에서 사용하는 용어의 정의는 다음 각 호와 같습니다.<br/>
          1. 이용자 : 본 약관에 따라 <span class="purple--text ">{{ appNm }}</span>의 제공하는 서비스를 받는 자<br/>
          2. 이용계약 : 서비스 이용과 관련하여 회사와 이용자간에 체결하는 계약<br/>
          3. 가입 : <span class="purple--text">{{ appNm }}</span>의 제공하는 신청서 양식에 해당 정보를 기입하고, 본 약관에 동의하여 서비스 이용계약을 완료시키는 행위<br/>
          4. 회원 : 당 사이트에 회원가입에 필요한 개인정보를 제공하여 회원 등록을 한 자<br/>
          5. 이용자번호(ID) : 회원 식별과 회원의 서비스 이용을 위하여 이용자가 선정하고 <span class="purple--text ">{{ appNm }}</span>의 승인하는 영문자와 숫자의 조합(하나의 주민등록번호에 하나의 ID만 발급 가능함)<br/>
        ...
        <p class="mt-4 mb-0" ref="PAGE2">제2장 서비스 이용계약</p>
        <span>
          제5조(이용계약의 성립)<br/>
          ...
          5. 기타 <span class="purple--text ">{{ appNm }}</span>의 정한 이용신청 요건에 미비 되었을 때<br/>
          제8조(계약사항의 변경)<br/>
          회원은 이용신청시 기재한 사항이 변경되었을 경우에는 수정하여야 하며, 수정하지 아니하여 발생하는 문제의 책임은 회원에게 있습니다.<br/>
        </span>
        <p class="mt-4 mb-0" ref="PAGE3">제3장 계약당사자의 의무</p>
        <span>
          제9조(회사의 의무)<br/>
        ...
        <p class="mt-4 mb-0" ref="PAGE4">제4장 서비스 이용</p>
        <span>
          제11조(회원의 의무)<br/>
        ...
        <p class="mt-4 mb-0" ref="PAGE5">제5장 계약해지 및 이용제한</p>
        <span>
          제17조(계약해지 및 이용제한)<br/>
        ...
        <p class="mt-4 mb-0" ref="PAGE6">제6장 기 타</p>
        <span>
          제18조(양도금지)<br/>
          ...
          부 칙<br/>
          (시행일) 이 약관은 2014년 11월 1일부터 시행합니다.<br/>
        </span>
      </v-card>
        <v-fade-transition>
          <v-btn
